...
const Records = (props) => {
  ...
  const [img,setImg] = useState(toDataUrl(user.filePhotograph,function(myBase64) {setImg(myBase64.toString())}));
  const [signature,setSignature] = useState(toDataUrl(user.parentSign,function(myBase64) {setSignature(myBase64.toString())}));
  const [childSign,setChildSign] = useState(toDataUrl(user.imgSign,function(myBase64) {setChildSign(myBase64.toString())}))
  ...
  return (
    <div className='w-full grid xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 items-center p-2 justify-between bg-white  rounded-md'>
        ...
        <div className='flex items-center justify-end'>
          <Button onClick={printPdf} color="greenary" size="small" sx={{width:"30%",color:"#FFF"}} variant="contained">Print</Button>
